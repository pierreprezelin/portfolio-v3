...
<li class="flex flex-col items-center gap-5 mobile:flex-row tablet:gap-10">
	...
	>
		<a href={localizeHref(`/blog/${post.slug}`, { locale: getLocale() })} title={m.go_to_post()} aria-label={m.go_to_post()}>
			{#if post.banner}
	...
	<div class="flex flex-col">
		...
		<p>
			<a href={localizeHref(`/blog/${post.slug}`, { locale: getLocale() })} class="flex items-center gap-2.5">
